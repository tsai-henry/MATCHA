...
from teleop.robot_control.robot_hand_inspire_newgen import InspireControllerNewGen
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hand', type=str, choices=['inspire1_newgen'], help='Select hand controller')
    ...
    # have to comment out because of dependency conflict with inspire_sdk (used for new hands)
    # if args.hand == "dex3":
    ...
    if args.hand == "inspire1_newgen":
        left_hand_array = Array('d', 75, lock = True)          # [input]
        right_hand_array = Array('d', 75, lock = True)         # [input]
        dual_hand_data_lock = Lock()
        dual_hand_state_array = Array('d', 12, lock = False)   # [output] current left, right hand state(12) data.
        dual_hand_action_array = Array('d', 12, lock = False)  # [output] current left, right hand action(12) data.
        hand_ctrl = InspireControllerNewGen(left_hand_array, right_hand_array, dual_hand_data_lock, dual_hand_state_array, dual_hand_action_array)
    else:
        pass
        ...
        if user_input.lower() == 'r':
            ...
            while running:
                ...
                if args.record:
                    ...
                    if args.hand == "inspire1" or args.hand == "inspire1_newgen":
